...
// AUDIT FIX (consolidation): basic-salary/OT used to be recomputed inline
// here with the same numbers as everywhere else — now uses the single
...
function SalaryCalculator() {
  ...
  const basic    = calculateBasicFromGross(t, f + m + tr);
  const house    = basic / 2;
  const daily    = basic / 30;
  const dailyG   = t / 30;
  const overtime = calculateHourlyOvertimeRate(basic);
...
function CompensationCalculator() {
  ...
  const m  = parseFloat(medical) || 0;
  const tr = parseFloat(transport) || 0;
  const basic      = calculateBasicFromGross(w, f + m + tr);
