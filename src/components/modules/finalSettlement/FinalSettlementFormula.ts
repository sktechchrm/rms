...
export const calculateWageComponents = (
  ...
  // AUDIT FIX (consolidation): basic and OT now come from the single shared
  // formula in utils/sharedFormulas.ts — see that file for why the divisor/
  // hours/multiplier are parameters, not hardcoded here.
  const basic = calculateBasicFromGross(totalMonthlyWage, foodAllowance + medicalAllowance + transportAllowance);
  const house = basic * DEFAULT_HOUSE_RENT_PERCENTAGE;
  const dailyBasic = basic / 30;
  const dailyGross = totalMonthlyWage / 30;
  const hourlyOvertimeRate = sharedHourlyOvertimeRate(basic);
...
export const calculateLastMonthOvertime = (monthlyBasic: number, payableHours: number): number => {
  const hourlyOvertimeRate = sharedHourlyOvertimeRate(monthlyBasic);
...
export const calculateHourlyOvertimeRate = (monthlyBasic: number): number => {
  return parseFloat(sharedHourlyOvertimeRate(monthlyBasic).toFixed(2));
};
