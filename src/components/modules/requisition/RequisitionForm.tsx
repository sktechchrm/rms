          ...
          <div style={{
            ...
            <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
              ...
              {/* Point 1 — Global Standard Templates: pick which print
                 layout this requisition uses. Defaults to 'standard' —
                 nothing changes for anyone who doesn't touch this. */}
              <select
                value={requisition.template ?? 'standard'}
                onChange={e => setRequisition({ ...requisition, template: e.target.value as RequisitionData['template'] })}
                title="Print template"
                aria-label="Print template"
                style={{
                  padding: '4px 10px', fontSize: 12, fontWeight: 600,
                  border: '1px solid #d1d5db', borderRadius: 20,
                  fontFamily: font, cursor: 'pointer', background: '#fff', color: '#374151',
                }}
              >
                {REQUISITION_TEMPLATE_OPTIONS.map(opt => (
                  <option key={opt.value} value={opt.value} title={opt.descEn}>
                    {opt.labelEn}
                  </option>
                ))}
              </select>
