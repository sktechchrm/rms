  ...
  return (
    <>
      ...
      <ModuleShell
        ...
        // Point 3 — Global Employee Search: unlike the single-employee
        // modules, this form holds an ARRAY of employee rows, so a search
        // result can't just overwrite "the form" — it prepends a new row,
        // following the exact same convention as the existing addEmployee()
        // button in Incrementform.tsx (prepend + renumber slNo). Also runs
        // the same serviceAge/basicSalary auto-calc addEmployee's manual
        // entry path would trigger, so the new row arrives fully computed
        // rather than needing the user to re-trigger onChange handlers.
        onEmployeeSelect={emp => {
          setIncrement(p => {
            const joiningDate = String(emp.joiningDate ?? '');
            const gross       = String(emp.grossSalary ?? '');
            const food        = emp.foodAllowance      !== undefined ? String(emp.foodAllowance)      : undefined;
            const medical     = emp.medicalAllowance   !== undefined ? String(emp.medicalAllowance)   : undefined;
            const transport   = emp.transportAllowance !== undefined ? String(emp.transportAllowance) : undefined;
            const newEmp: EmployeeIncrement = {
              slNo: 1,
              employeeName: String(emp.fullNameBengali ?? emp.fullName ?? ''),
              employeeId:   String(emp.idNo ?? emp.cardNo ?? ''),
              designation:  String(emp.designation ?? ''),
              department:   String(emp.sectionLine ?? emp.department ?? ''),
              dateOfJoining: joiningDate,
              serviceAge:   calculateServiceAge(joiningDate),
              basicSalary:  calculateBasicSalary(gross, food, medical, transport),
              grossSalary:  gross,
              foodAllowance: food,
            ...
            const reNumbered = [newEmp, ...p.employees].map((e, i) => ({ ...e, slNo: i + 1 }));
            return { ...p, employees: reNumbered };
          });
        }}
