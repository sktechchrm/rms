...
// FIXED: educationHistory[] and previousJobs[] arrays replace flat fields
// import path corrected to ../../../types/employee.types

import React from 'react';
import { FaUser } from 'react-icons/fa';
import { EmployeeFormData, AgeData, EducationEntry, PreviousJobEntry } from './employee.types';
...
interface DocumentProps { formData: EmployeeFormData; }
    ...
    <div className="space-y-6">
      <div className="border-2 border-gray-800 p-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div><p className="text-sm text-gray-600">Employee Name:</p><p className="font-semibold">{formData.fullName}</p></div>
          <div><p className="text-sm text-gray-600">Employee ID:</p><p className="font-semibold">{formData.employeeId}</p></div>
          <div><p className="text-sm text-gray-600">Designation:</p><p className="font-semibold">{formData.designation}</p></div>
          <div><p className="text-sm text-gray-600">Department:</p><p className="font-semibold">{formData.department}</p></div>
        </div>
      </div>
      <div className="border-2 border-gray-800 p-6">
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div><p className="text-sm text-gray-600">Nominee Name:</p><p className="font-semibold">{formData.nomineeName}</p></div>
            <div><p className="text-sm text-gray-600">Relation with Employee:</p><p className="font-semibold">{formData.nomineeRelation}</p></div>
          </div>
          <div><p className="text-sm text-gray-600">NID Number:</p><p className="font-semibold">{formData.nomineeNid}</p></div>
          <div><p className="text-sm text-gray-600">Address:</p><p className="font-semibold">{formData.nomineeAddress}</p></div>
          <div><p className="text-sm text-gray-600">Percentage of Share:</p><p className="font-semibold">{formData.nomineePercentage}%</p></div>
        </div>
      </div>
      <div className="mt-12">
        ...
        <div className="flex justify-between mt-16">
          <div><div className="border-t-2 border-black w-48"></div><p className="text-sm mt-2">Employee Signature</p><p className="text-sm">{formData.fullName}</p></div>
          <div><div className="border-t-2 border-black w-48"></div><p className="text-sm mt-2">Nominee Signature</p><p className="text-sm">{formData.nomineeName}</p></div>
        </div>
      </div>
    </div>
  </div>
    ...
    <div className="bg-white p-8 max-w-4xl mx-auto">
      ...
      <div className="border-2 border-gray-800 p-8 space-y-6">
        ...
        <div className="grid grid-cols-2 gap-6">
          <div className="bg-gray-100 p-4 rounded"><p className="text-sm text-gray-600">Date of Birth:</p><p className="text-xl font-bold">{formatDate(formData.dateOfBirth)}</p></div>
          <div className="bg-gray-100 p-4 rounded"><p className="text-sm text-gray-600">As on Today:</p><p className="text-xl font-bold">{new Date().toLocaleDateString('en-GB')}</p></div>
        </div>
        <div className="bg-blue-50 border-2 border-blue-300 p-6 rounded-lg text-center">
          ...
          <div className="flex justify-center gap-8 text-center">
            <div><p className="text-4xl font-bold text-blue-600">{age.years}</p><p className="text-sm">Years</p></div>
            <div><p className="text-4xl font-bold text-blue-600">{age.months}</p><p className="text-sm">Months</p></div>
            <div><p className="text-4xl font-bold text-blue-600">{age.days}</p><p className="text-sm">Days</p></div>
          </div>
        </div>
  ...
  <div className="bg-white p-8 max-w-2xl mx-auto">
    <div className="border-4 border-blue-600 rounded-lg overflow-hidden">
      ...
      <div className="bg-white p-6 border-t-4 border-blue-600">
        ...
        <p className="text-xs text-center mt-4 text-gray-600">If found, please return to the above address</p>
      </div>
    </div>
  </div>
);
  ...
  <div className="bg-white p-8 max-w-4xl mx-auto text-gray-900 print:p-0">
    ...
    <div className="space-y-4">
      ...
      <div className="border border-gray-800 p-3">
        ...
        <div className="space-y-3 text-sm">
          ...
          <div className="border-t pt-2">
            ...
            <p>{formData.presentAddress}</p>
            <p className="text-xs text-gray-600">Union: {formData.presentUnion}, Village: {formData.presentVillage}, P.O: {formData.presentPostOffice}, Thana: {formData.presentThana}, Dist: {formData.presentDistrict}, Div: {formData.presentDivision}</p>
          </div>
          <div className="border-t pt-2">
            <p className="font-semibold mb-1 italic">Permanent Address:</p>
            <p>{formData.permanentAddress}</p>
            <p className="text-xs text-gray-600">Union: {formData.permanentUnion}, Village: {formData.permanentVillage}, P.O: {formData.permanentPostOffice}, Thana: {formData.permanentThana}, Dist: {formData.permanentDistrict}, Div: {formData.permanentDivision}</p>
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-2 gap-y-2 text-sm mb-3">
          ...
          <div className="col-span-2 border-t mt-1 pt-1">
            {formData.educationHistory && formData.educationHistory.length > 0
              ? formData.educationHistory.map((ed: EducationEntry, i: number) => (
                <div key={ed.id ?? i} className={i > 0 ? 'mt-1.5 pt-1.5 border-t border-dashed' : ''}>
      ...
      <div className="border border-gray-800 p-3">
        ...
        {formData.previousJobs && formData.previousJobs.length > 0
          ? formData.previousJobs.map((job: PreviousJobEntry, i: number) => (
            <div key={job.id ?? i} className={`grid grid-cols-2 gap-y-2 text-sm ${i > 0 ? 'mt-2 pt-2 border-t border-dashed' : ''}`}>
              <div><strong>Company:</strong> {job.prevCompanyName}</div>
              <div><strong>Service Years:</strong> {job.prevServiceYears}</div>
              <div><strong>Designation:</strong> {job.prevDesignation}</div>
              <div><strong>Reason for Leave:</strong> {job.prevLeaveReason}</div>
              <div className="col-span-2"><strong>Reference:</strong> {job.prevRefDetails}</div>
            </div>
          ))
      ...
      <div className="border border-gray-800 p-3">
        ...
        <div className="grid grid-cols-2 gap-y-2 text-sm">
          ...
          <div className="col-span-2 border-t mt-1 pt-1">
            ...
            <br /><strong>Address:</strong> {formData.nomineeAddress}
          </div>
        </div>
      </div>
      ...
      <div className="border border-gray-800 p-3">
        ...
        <div className="grid grid-cols-2 gap-y-2 text-sm">
          ...
          <div className="col-span-2 border-t mt-1 pt-1">
            ...
            <br /><strong>Supervisor Phone:</strong> {formData.supervisorPhone} | <strong>Relation:</strong> {formData.supervisorRelation}
          </div>
        </div>
      </div>
    </div>
...
export const LeftWorkerNotice: React.FC<DocumentProps> = ({ formData }) => (
  <div className="bg-white p-8 max-w-4xl mx-auto">
    ...
    <div className="space-y-4 text-justify">
      <p>Dear {formData.fullName},</p>
      <p>It has come to our attention that you have been absent from your duties without prior permission or intimation to the management.</p>
      <p><strong>Details of Absence:</strong></p>
      <ul className="list-disc ml-8 space-y-2">
        <li>Last Working Day: {formatDate(formData.joiningDate)}</li>
        <li>Days of Unauthorized Absence: _____ days</li>
        <li>Notice Issued Date: {new Date().toLocaleDateString('en-GB', { day: '2-digit', month: 'long', year: 'numeric' })}</li>
      </ul>
      <p>You are hereby required to submit a written explanation within <strong>7 (seven) days</strong> from the date of receipt of this notice.</p>
      <p className="font-semibold text-red-600" role="alert">Failure to respond to this notice within the specified time will be treated as an admission of misconduct.</p>
    </div>
    <div className="mt-16 flex justify-between">
      <div>
    ...
    <div className="mt-8 border-t-2 border-gray-400 pt-4">
      ...
      <div className="flex justify-between items-end">
        ...
        <div><p className="text-sm">Date: ______________</p></div>
      </div>
    </div>
  </div>
);