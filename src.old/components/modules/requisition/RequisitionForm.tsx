...
// ── AmountField MUST be outside the parent component ─────────────────────────
// Defining it inside causes React to remount it on every render (new function
// reference = new component type), resetting the input after each keystroke.
                  ...
                  <tr key={index} style={{ background: index % 2 === 0 ? '#fff' : '#fafafa' }}>
                    ...
                    {!isTaka && (
                      <>
                        ...
                        <td style={{ ...tdS, width: 110 }}>
                          <textarea value={item.quantity} onChange={e => handleItemChange(index, 'quantity', e.target.value)} placeholder="e.g. 10" rows={1} style={ta} />
                        </td>
                    ...
                    {isTaka && (
                      <>
                        ...
                        <td style={{ ...tdS, width: 130 }}>
                          <AmountField value={item.amount} onChange={v => handleAmountChange(index, v)} />
                        </td>
                    ...
                    <td style={{ ...tdS, borderRight: 'none', textAlign: 'center', width: 44 }}>
                      {requisition.items.length > 1 && (
                        <button
                          ...
                          style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#ef4444', fontSize: 16, lineHeight: 1, padding: '2px 4px', borderRadius: 4, display: 'inline-flex', alignItems: 'center', justifyContent: 'center' }}
                        >✕</button>
                      )}
                    </td>
                  </tr>
