...
export default function IncrementFormComponent({ increment, setIncrement }: IncrementFormProps) {
  ...
  const handleEmployeeChange = (index: number, field: keyof EmployeeIncrement, value: string) => {
    ...
    if (field === 'grossSalary') {
      // AUDIT FIX: now passes the row's real allowances when Global Employee
      // Search has populated them; calculateBasicSalary() falls back to the
      // same defaults as before when they're not set (manual entry).
      const row = newEmployees[index];
      newEmployees[index].basicSalary = calculateBasicSalary(
        value, row.foodAllowance, row.medicalAllowance, row.transportAllowance,
      );
    }
